...
test('migrateV5toV6 marks the epoch without inventing a variety stamp',()=>{
  const route=initRoute(SEED);delete route.variety;   /* model a genuine pre-v6 save */
  const v5=migrateV4toV5(migrateV3toV4(v3env(route)));
  assert.equal(v5.saveVersion,5);
  assert.equal(v5.run.schemaVersion,5);
  ...
  /* a resumed pre-v6 run deliberately has no variety stamp, so its fights stay
     byte-identical to what it scouted */
  assert.equal(v6.run.route.variety,undefined,'migration does not turn variety on for old runs');
  assert.deepEqual(v6.run.economy,v5.run.economy,'economy untouched');

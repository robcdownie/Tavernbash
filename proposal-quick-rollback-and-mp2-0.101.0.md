...
1. **Calibration corpus (eligibility defined precisely, per the Codex correction):** the Cloud Ledger run records already carry the needed behavioral events per market visit: `shop_roll`, `shop_buy` (with offer and cost), `shop_reroll`, `shop_freeze`, `tier_up`, `fusion`, sells, and per-phase timing. Extract per-market and per-run behavioral marginals from at least 30 eligible runs. ELIGIBILITY: behavioral fitting may combine runs from different builds ONLY when those builds' market rules (pool filters, weights, prices, freeze and reroll semantics) and telemetry schema are proven identical, by diffing the relevant source and schema between the versions; any run from a build failing that proof is excluded from fitting. The held-out clear-rate validation set is stricter still: it may use only runs whose difficulty constants AND route mode match the simulated baseline exactly. The marginals: buys per market (distribution), rerolls per market, freeze rate per market, sell rate, tier-by-district curve, distinct-line count over time, vault occupancy, share of buys that continue a line versus open one, enchant-buy rate.
...
3. **Validation, not tuning:** on the held-out eligible runs (same difficulty constants and mode as the simulated baseline), mp-2's predicted clear rate is compared to the human sample's. Pre-registered acceptance: behavioral marginals within tolerance AND the clear-rate prediction within 10 points of the held-out human rate. THE FROZEN FLOORS REMAIN AUTHORITATIVE BY DEFAULT (the Codex correction): a validated mp-2 gives Robbie evidence he may weigh toward an EXPLICIT amendment of the 60, 40, and 90 floors, and it creates no presumption that those floors will change; absent his amendment they stand as written.

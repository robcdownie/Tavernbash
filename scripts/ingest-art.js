...
export function targetFor(filename) {
  ...
  for (const id of UI_IDS) if (tokens.has(id)) hits.push({dir:'ui', name:id + '.png', kind:'icon'});
  /* magma heart is the ware; heart alone is the UI badge */
  if (tokens.has('magma') && tokens.has('heart')) {
    return {dir:'items', name:'magma.png', kind:'icon'};
  }
